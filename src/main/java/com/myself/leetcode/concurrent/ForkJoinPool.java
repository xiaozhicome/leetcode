...
public class ForkJoinPool extends AbstractExecutorService {
     ...
     * properly fence the accesses, and possibly thread waiting
     ...
     * waiting for a thread), or SCANNING for tasks (when neither hold
     ...
     * stronger than minimal ordering because we must sometimes thread
    ...
    final int helpComplete(WorkQueue w, CountedCompleter<?> task,
        ...
        if ((ws = workQueues) != null && (m = ws.length - 1) >= 0 &&
            ...
            int r = w.hint ^ w.top;              // arbitrary seed for map
